...
fn ability_available(
    ...
    // Per-ability suppression (Disruptor Flute, etc.): check activatable flag.
    if !ability.activatable {
        return false;
    }
    // Action-Restriction on the source permanent (Null Rod / Karn: "artifact
    // abilities can't be activated"). Keyed on the source, so it covers every
    // ability the source has, incl. granted ones — CR 101.2 "can't beats can".
